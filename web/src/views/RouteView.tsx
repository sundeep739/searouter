...
export default function RouteView({ map, active }: { map: MapCtl; active: boolean }) {
  ...
  const run = useCallback(
    ...
    [avoid, speed, waypoints, mode, etd, etaIn],
  )
  ...
  }, [result, selected, areas, draft, waypoints, origin, dest, map, active])

  const crossing = useMemo(() => {
    if (!result || areas.length === 0) return false
    const line =
      selected === -1 && result.custom ? result.custom.coords : result.variants[selected]?.coords
    ...
    <>
      <div className="space-y-3">
          ...
          <div className="mt-2 flex flex-wrap gap-1.5">
            {Object.entries(PASSAGES).map(([id, label]) => (
              <Chip
                ...
                onClick={() =>
                  setAvoid((a) => (a.includes(id) ? a.filter((x) => x !== id) : [...a, id]))
                }
              >
                {label}
              </Chip>
        ...
        <div className="flex gap-2">
          <Btn
            className="flex-1"
            disabled={!origin || !dest || busy}
            onClick={() => origin && dest && run(origin.unlocode, dest.unlocode)}
          >
            ...
            <Btn
              ...
              onClick={async () => {
                await saveItem(
                  session.user.id,
                  'route',
                  `${result.origin.unlocode} → ${result.dest.unlocode}`,
                  { origin: result.origin.unlocode, dest: result.dest.unlocode, avoid, speed },
                )
        ...
        {drawMode !== 'none' && (
          <p className="text-[11px] text-slate-400">
            Tap the map to {drawMode === 'waypoint' ? 'drop waypoints — the route will pass through them (recalculate after adding)' : 'outline an area, then close it'}.
          </p>
        )}
        ...
        {crossing && (
          <ErrorNote>
            The selected route crosses your avoid area(s) — drop waypoints to steer around them,
            then recalculate.
          </ErrorNote>
        )}
      </div>
        ...
        <div className="space-y-2">
          ...
          {result.schedule && (
            <Card className="!p-3 text-sm">
              ...
              <b className="text-sea-600 dark:text-sea-400">
                {result.schedule.required_speed_knots} kn
              </b>{' '}
              average.
            </Card>
          )}
